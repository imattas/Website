...
def main() -> None:
  ...
  glyphs = {char: index for index, char in enumerate(char for char in chars if ord(char) in cmap)}
  ...
  for root in CONTENT_ROOTS:
    ...
    for path in base.rglob("*"):
      if path.is_file() and path.suffix in TEXT_EXTENSIONS:
        chars.update(match.group(0) for match in re.finditer(r"[\u3000-\u303f\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff00-\uffef]", path.read_text()))
...
def write_atlas(font: TTFont, glyphs: dict[str, int], ppem: int, output: Path) -> None:
  strike_index = next(
    index for index, strike in enumerate(font["EBLC"].strikes) if strike.bitmapSizeTable.ppemY == ppem
  )
