...
- [OpenRC init system](https://github.com/OpenRC/openrc) — how runlevel service symlinks relate to `/etc/init.d/` scripts
- [Alpine Linux](https://www.alpinelinux.org/) — the distribution used in the disk image
- `mount(8)` / `sha1sum(1)` — read-only loopback mounting and hashing for forensic integrity
